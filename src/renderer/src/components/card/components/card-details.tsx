...
import {
  ...
  DropdownMenuItem,
  DropdownMenuTrigger
} from '@/components/dropdown/ui/dropdown'
...
export function DetailTaskCard() {
  ...
  const availableMembers = useMemo(
    () => allMembers.filter((member) => !assignees.some((assigned) => assigned.id === member.id)),
    [assignees]
  )
  ...
  return (
    <div className="">
      <Card className="group relative w-full overflow-hidden rounded-2xl border border-border/40 bg-background/60 text-foreground backdrop-blur transition-all hover:border-border/60 hover:shadow-lg">
        <div className="pointer-events-none absolute inset-0 bg-linear-to-br from-foreground/4 via-transparent to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100 -z-10" />
        ...
        <CardContent className="space-y-10 px-6 py-8">
          ...
          >
            <div className="space-y-3">
              ...
              />
              <p id="task-title-description" className="text-xs text-foreground/60">
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs text-foreground/60">{priorityMap[priority].description}</p>
            </div>
          </motion.div>
                ...
                {assignees.map((member) => (
                  ...
                  >
                    ...
                    <div className="flex flex-col text-left">
                      ...
                      <span className="text-xs text-foreground/60">{member.role}</span>
                    </div>
                    <Button
                      variant="ghost"
                      size="icon"
          ...
          <div className="flex flex-col gap-4 rounded-2xl border border-border/40 bg-background/40 px-4 py-4 backdrop-blur md:flex-row md:items-center md:justify-between">
            <div className="md:flex space-y-2 md:space-y-0 items-center gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-foreground">Reminder Task</p>
