...
export function Availability({
  ...
  const weekStart = React.useMemo(() => {
    ...
    // Adjust to Monday start: if dow is 0 (Sunday), go back 6 days; otherwise go back dow-1 days
      ...
      >
        <div className="flex-1 min-h-0 overflow-x-auto overflow-y-hidden">
          <div className="min-w-max">
            {/* Header */}
            <div
              className="sticky top-0 z-10 grid w-full min-w-max border-b border-primary-alt/15 bg-secondary-background"
              style={{ gridTemplateColumns: `64px repeat(${days.length}, minmax(100px, 1fr))` }}
            >
              <div className="border-r border-primary-alt/15 p-2 text-sm font-bold text-primary-alt">
              ...
              {days.map((dayIndex, i) => {
                ...
                return (
                  <div
                    key={dayIndex}
                    className="border-r border-primary-alt/15 px-2 py-2 text-center text-md font-bold text-primary-alt last:border-r-0"
                  >
...
function DayColumn({
  ...
  return (
    <div
      ...
      className="relative border-r border-primary-alt/15 last:border-r-0 h-full"
...
function DraggableTimeSpan({
  ...
  return (
    ...
    >
      {/* Resize Handle Top */}
      <div
        className="absolute top-0 left-0 right-0 h-1.5 cursor-n-resize hover:bg-secondary-background/70"
      ...
      <div
        className="absolute bottom-0 left-0 right-0 h-1.5 cursor-s-resize hover:bg-secondary-background/70"
  ...
  return (
    <div
      className={cn(
        ...
        isOverlay && 'rounded-md border bg-primary p-2 text-primary-foreground shadow-lg w-30'
      )}
    >
      <div className="flex items-start justify-between">
        ...
        {onDelete && (
          <Button
            variant="ghost"
            size="icon"
            className="h-5 w-5 text-primary-alt hover:text-primary-alt hover:bg-secondary-background/70 -mt-1 -mr-1"
            onMouseDown={(e) => {
